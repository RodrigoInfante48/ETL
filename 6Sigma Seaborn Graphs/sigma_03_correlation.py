"""
sigma_03_correlation.py
========================
6 Sigma Analysis - Block 3: Correlation & Regression Analysis
...
for ax, data, title in zip(axes,
                            [df_num, df_log],
                            ["Pearson Correlation (Raw)", "Pearson Correlation (Log-transformed)"]):
...
countries_plot = df_pair["country_name"].unique()
palette_map = dict(zip(countries_plot, sns.color_palette("tab10", len(countries_plot))))
...
g.figure.suptitle("6 Sigma — Block 3: Pair Plot (GDP/Capita, Inflation, Unemployment)",
                  fontsize=13, fontweight="bold", y=1.01)

# Rename axes labels
new_labels = {"gdp_per_capita_usd": "GDP/Capita (USD)",
              "inflation_annual_pct": "Inflation (%)",
...
# ── Figure 3: Scatter + Regression (GDP/Capita vs Unemployment) ───────────
...
# ── Figure 4: Correlation significance matrix ──────────────────────────────
...
# Annotation: r + significance stars
