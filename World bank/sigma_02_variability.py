"""
sigma_02_variability.py  —  6 Sigma Block 2: Variability, Boxplots & Outliers
...
fig.suptitle("6 Sigma — Block 2: Variability & Spread by Country",
             fontsize=15, fontweight="bold")
for ax, (col, label) in zip(axes, INDICATORS.items()):
    order = df.groupby("country_name")[col].median().sort_values().index.tolist()
    sns.boxplot(data=df, x=col, y="country_name", order=order, palette=PALETTE, ax=ax,
    ...
    ax.set_xlabel(label); ax.set_ylabel(""); ax.legend(fontsize=8)
plt.tight_layout()
...
fig2.suptitle("6 Sigma — Block 2: Density Distribution per Country (Violin)",
              fontsize=15, fontweight="bold")
...
# ── Figure 3: Outlier Detection (IQR + 3σ) ────────────────────────────────
fig3, axes3 = plt.subplots(1, 3, figsize=(22, 7))
fig3.suptitle("6 Sigma — Block 2: Outlier Detection (IQR + 3σ Rule)",
              fontsize=15, fontweight="bold")
for ax, (col, label) in zip(axes3, INDICATORS.items()):
    data = df[["country_name","year",col]].dropna().copy()
    mean = data[col].mean(); std = data[col].std()
    q1 = data[col].quantile(0.25); q3 = data[col].quantile(0.75); iqr = q3 - q1
    lower_iqr = q1 - 1.5*iqr; upper_iqr = q3 + 1.5*iqr
    lower_3s  = mean - 3*std;  upper_3s  = mean + 3*std
    ...
    ax.scatter(normal["year"],  normal[col],  color="steelblue", alpha=0.5, s=20, label="Normal")
    ax.scatter(out_iqr["year"], out_iqr[col], color="orange",    alpha=0.8, s=50, marker="D",
               label=f"IQR Outlier (n={len(out_iqr)})")
    ax.scatter(out_3s["year"],  out_3s[col],  color="red",       alpha=0.9, s=80, marker="X",
               label=f"3σ Outlier (n={len(out_3s)})")
    ax.axhline(mean,      color="blue",   linestyle="--", linewidth=1, label=f"μ={mean:.2f}")
    ax.axhline(upper_3s,  color="red",    linestyle=":",  linewidth=1)
    ax.axhline(lower_3s,  color="red",    linestyle=":",  linewidth=1)
    ax.axhline(upper_iqr, color="orange", linestyle="-.", linewidth=1)
    ax.axhline(lower_iqr, color="orange", linestyle="-.", linewidth=1)
    ax.set_title(label, fontsize=12, fontweight="bold")
    ax.set_xlabel("Year"); ax.set_ylabel(label); ax.legend(fontsize=7)
...
# ── Figure 4: Coefficient of Variation heatmap ────────────────────────────
...
ax4.set_title("6 Sigma — Block 2: Coefficient of Variation\n(Higher = More Variability)",
