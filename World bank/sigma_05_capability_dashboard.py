"""
sigma_05_capability_dashboard.py  —  6 Sigma Block 5: Cp/Cpk + Executive Dashboard
...
SPEC_LIMITS = {
    "gdp_per_capita_usd":  {"LSL": 1_000,  "USL": 80_000, "target": 15_000},
    "inflation_annual_pct":{"LSL": -2.0,   "USL": 10.0,   "target": 3.0},
    "unemployment_pct":    {"LSL": 0.0,    "USL": 15.0,   "target": 5.0},
}

def capability_metrics(data, lsl, usl, target):
    mu = data.mean(); sd = data.std(ddof=1)
    if sd == 0: return None
    cp  = (usl - lsl) / (6 * sd)
    cpu = (usl - mu)  / (3 * sd)
    cpl = (mu  - lsl) / (3 * sd)
...
fig1.suptitle("6 Sigma — Block 5: Process Capability (Cp / Cpk)",
...
for ax, (col, label) in zip(axes1, INDICATORS.items()):
    ...
    ax.fill_between(x_range, pdf_vals, where=(x_range < sl["LSL"]),
                    color="red", alpha=0.35, label="Below LSL")
    ax.fill_between(x_range, pdf_vals, where=(x_range > sl["USL"]),
                    color="red", alpha=0.35, label="Above USL")
    ax.axvline(sl["LSL"],    color="red",    linestyle="--", linewidth=2, label=f"LSL={sl['LSL']:,.0f}")
    ax.axvline(sl["USL"],    color="red",    linestyle="--", linewidth=2, label=f"USL={sl['USL']:,.0f}")
    ax.axvline(sl["target"], color="purple", linestyle="-",  linewidth=1.8, label=f"Target={sl['target']:,.0f}")
    ax.axvline(cm["mean"],   color="navy",   linestyle="-",  linewidth=2, label=f"μ={cm['mean']:,.2f}")
    ax.axvline(cm["mean"]+3*cm["std"], color="orange", linestyle=":", linewidth=1.5, label="±3σ")
    ...
    info = (f"Cp   = {cm['Cp']:.3f}\n"
            ...
            f"μ = {cm['mean']:,.2f}\n"
            f"σ = {cm['std']:,.2f}")
    ax.text(0.02, 0.97, info, transform=ax.transAxes, fontsize=9, va="top",
            color=color_cpk, fontweight="bold",
            bbox=dict(boxstyle="round,pad=0.4", facecolor="white",
                      edgecolor=color_cpk, alpha=0.9))
    ax.set_title(label, fontsize=12, fontweight="bold")
    ax.set_xlabel(label); ax.set_ylabel("Probability Density"); ax.legend(fontsize=7)
    capability_summary.append({"Indicator": label, "Cp": cm["Cp"], "Cpk": cm["Cpk"],
...
# ── Figure 2: Sigma Level Scorecard ───────────────────────────────────────
sigma_rows = []
for country in df["country_name"].unique():
    ...
    for col, label in INDICATORS.items():
        data = sub[col].dropna()
        sl = SPEC_LIMITS[col]
        cm = capability_metrics(data, sl["LSL"], sl["USL"], sl["target"]) if len(data) >= 5 else None
...
sns.heatmap(sigma_df.clip(-1, 6), annot=sigma_df.round(2), fmt="g",
            cmap="RdYlGn", vmin=-1, vmax=6, ax=ax2,
            linewidths=0.5, linecolor="white",
            cbar_kws={"label": "Sigma Level (Cpk × 3)"})
ax2.set_title("6 Sigma — Block 5: Sigma Level Scorecard\n(Green ≥ 4σ | Yellow 2–4σ | Red < 2σ)",
...
# ── Figure 4: Executive Dashboard (9 panels) ──────────────────────────────
fig4 = plt.figure(figsize=(24, 20))
fig4.suptitle("6 Sigma Executive Dashboard — World Bank Economic Indicators (2000–2023)",
              fontsize=17, fontweight="bold", y=0.98)
gs = gridspec.GridSpec(3, 3, figure=fig4, hspace=0.45, wspace=0.35)
PALETTE_C = sns.color_palette("tab10", df["country_name"].nunique())
country_color = dict(zip(df["country_name"].unique(), PALETTE_C))

# Panel 0,0: GDP trend
ax00 = fig4.add_subplot(gs[0, 0])
for country, color in country_color.items():
    sub = df[df["country_name"] == country].sort_values("year")
    ax00.plot(sub["year"], sub["gdp_per_capita_usd"], linewidth=1.8, color=color,
...
pivot_inf = df.pivot_table(index="country_name", columns="year", values="inflation_annual_pct")
...
order_u = df.groupby("country_name")["unemployment_pct"].median().sort_values().index.tolist()
sns.boxplot(data=df, x="unemployment_pct", y="country_name", order=order_u,
            palette="Set2", ax=ax02, flierprops=dict(marker="o", color="red", markersize=3))
ax02.axvline(df["unemployment_pct"].mean(), color="red", linestyle="--", linewidth=1.5, label="Mean")
ax02.set_title("Unemployment Distribution", fontsize=11, fontweight="bold")
ax02.set_xlabel("Unemployment (%)"); ax02.set_ylabel(""); ax02.legend(fontsize=7)

# Panel 1,0: Capability table
ax10 = fig4.add_subplot(gs[1, 0])
ax10.axis("off")
cap_df = pd.DataFrame(capability_summary)
if not cap_df.empty:
    tbl_data = [[r["Indicator"], f"{r['Cp']:.2f}", f"{r['Cpk']:.2f}",
                 f"{r['Sigma Level']:.2f}", f"{r['DPMO']:,.0f}"]
                for _, r in cap_df.iterrows()]
    tbl = ax10.table(cellText=tbl_data,
                     colLabels=["Indicator","Cp","Cpk","Sigma","DPMO"],
    ...
    for i, row in enumerate(tbl_data, start=1):
        cpk_val = float(row[2])
        c = "#d5f5e3" if cpk_val >= 1.33 else ("#fef9e7" if cpk_val >= 1.0 else "#fadbd8")
        for j in range(5): tbl[i, j].set_facecolor(c)
ax10.set_title("Capability Summary", fontsize=11, fontweight="bold", pad=40)

# Panel 1,1: Scatter regression
ax11 = fig4.add_subplot(gs[1, 1])
ds = df[["gdp_per_capita_usd","unemployment_pct","country_name"]].dropna()
sns.scatterplot(data=ds, x="gdp_per_capita_usd", y="unemployment_pct",
                hue="country_name", alpha=0.45, s=20, ax=ax11, legend=False)
sns.regplot(data=ds, x="gdp_per_capita_usd", y="unemployment_pct",
            scatter=False, ax=ax11, color="red", line_kws={"linewidth": 2.5})
r, p = stats.pearsonr(ds["gdp_per_capita_usd"], ds["unemployment_pct"])
p_str = "p<0.001" if p < 0.001 else f"p={p:.4f}"
ax11.text(0.03, 0.92, f"r={r:.3f}  {p_str}", transform=ax11.transAxes,
...
# Panel 1,2: Correlation heatmap
ax12 = fig4.add_subplot(gs[1, 2])
num_cols = ["gdp_per_capita_usd","gdp_current_usd","inflation_annual_pct","unemployment_pct"]
labels_s = ["GDP/cap","GDP total","Inflation","Unemploy."]
...
corr = df_log.rename(columns=dict(zip(num_cols, labels_s))).corr()
mask = np.triu(np.ones_like(corr, dtype=bool))
sns.heatmap(corr, mask=mask, annot=True, fmt=".2f", cmap="coolwarm",
            vmin=-1, vmax=1, ax=ax12, linewidths=0.5, cbar_kws={"shrink": 0.7})
ax12.set_title("Correlation Matrix (log)", fontsize=11, fontweight="bold")

# Panel 2,0: EWMA inflation
ax20 = fig4.add_subplot(gs[2, 0])
LAMBDA = 0.2
yi = df.groupby("year")["inflation_annual_pct"].mean().dropna().reset_index().sort_values("year")
vals = yi["inflation_annual_pct"].values; yrs = yi["year"].values
ewma = np.zeros(len(vals)); ewma[0] = vals.mean()
for t in range(1, len(vals)): ewma[t] = LAMBDA*vals[t] + (1-LAMBDA)*ewma[t-1]
mu0 = vals.mean(); sig0 = vals.std()
ax20.plot(yrs, vals, "o", color="lightcoral", alpha=0.6, markersize=5, label="Observed")
ax20.plot(yrs, ewma, "o-", color="steelblue", linewidth=2, markersize=5, label=f"EWMA (λ={LAMBDA})")
ax20.axhline(mu0+3*sig0, color="red",   linestyle="--", linewidth=1.5, label="±3σ")
ax20.axhline(mu0-3*sig0, color="red",   linestyle="--", linewidth=1.5)
ax20.axhline(mu0,        color="green", linestyle="-",  linewidth=1.5, label=f"μ={mu0:.2f}%")
ax20.set_title("EWMA — Inflation (Global)", fontsize=11, fontweight="bold")
ax20.set_xlabel("Year"); ax20.set_ylabel("Inflation (%)"); ax20.legend(fontsize=7)

# Panel 2,1: CV heatmap
...
for col, label in INDICATORS.items():
    for country in df["country_name"].unique():
        ...
        if len(sub) > 2 and sub.mean() != 0:
            cv_rows.append({"Country": country, "Indicator": label,
                            "CV (%)": (sub.std() / abs(sub.mean())) * 100})
pivot_cv = pd.DataFrame(cv_rows).pivot(index="Country", columns="Indicator",
                                       values="CV (%)").fillna(0)
sns.heatmap(pivot_cv, annot=True, fmt=".1f", cmap="YlOrRd", ax=ax21,
            linewidths=0.5, cbar_kws={"label": "CV (%)", "shrink": 0.7})
ax21.set_title("Coefficient of Variation\n(Higher = Less Stable)", fontsize=11, fontweight="bold")
ax21.set_xlabel(""); ax21.set_ylabel("")

# Panel 2,2: Sigma level bar chart
ax22 = fig4.add_subplot(gs[2, 2])
if not sigma_df.empty:
    sigma_melt = sigma_df.reset_index().melt(id_vars="Country", var_name="Indicator", value_name="Sigma")
    sigma_melt = sigma_melt.dropna().assign(Sigma=sigma_melt.dropna()["Sigma"].clip(-1, 6))
    sns.barplot(data=sigma_melt, x="Sigma", y="Country", hue="Indicator",
                ax=ax22, palette="Set1", alpha=0.85)
    ax22.axvline(1.33, color="black", linestyle="--", linewidth=1.5, label="Cpk=1.33 (4σ)")
    ax22.axvline(1.0,  color="gray",  linestyle=":",  linewidth=1.2, label="Cpk=1.0 (3σ)")
    ax22.set_title("Sigma Level by Country & Indicator", fontsize=11, fontweight="bold")
    ax22.set_xlabel("Sigma Level"); ax22.set_ylabel(""); ax22.legend(fontsize=7)
...
print("\n" + "="*55)
print("  Block 5 complete — FULL 6 SIGMA SUITE DONE")
print("  17 charts generated across 5 blocks")
print("="*55)
