"""
sigma_01_descriptive.py  —  6 Sigma Block 1: Distribution & Normality
...
# ── Figure 1: Histogram + KDE + Q-Q per indicator ─────────────────────────
fig, axes = plt.subplots(2, 4, figsize=(20, 10))
fig.suptitle("6 Sigma — Block 1: Distribution & Normality Analysis",
...
for col_idx, (col, label) in enumerate(INDICATORS.items()):
    ...
    ax1.axvline(mean_val + std_val,   color="orange",  linestyle=":",  linewidth=1.5, label="+1σ")
    ax1.axvline(mean_val - std_val,   color="orange",  linestyle=":",  linewidth=1.5, label="-1σ")
    ax1.axvline(mean_val + 3*std_val, color="darkred", linestyle="-.", linewidth=1.2, label="+3σ")
    ax1.axvline(mean_val - 3*std_val, color="darkred", linestyle="-.", linewidth=1.2, label="-3σ")
...
for col, label in INDICATORS.items():
    ...
    summary_rows.append([
        label, f"{len(d):,}", f"{d.mean():,.2f}", f"{d.median():,.2f}",
        f"{d.std():,.2f}", f"{d.min():,.2f}", f"{d.max():,.2f}",
        f"{d.skew():.3f}", f"{d.kurtosis():.3f}", f"{p:.4f}",
...
for i, row in enumerate(summary_rows, start=1):
    c = "#d5f5e3" if row[-1] == "Normal" else "#fadbd8"
    table[i, -1].set_facecolor(c)
